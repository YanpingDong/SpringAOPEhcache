...
public class MethodCacheInterceptor implements MethodInterceptor,
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {   
        ...
        logger.debug("Find object from " + cache.getName() + " cache " );   
        String cacheKey = getCacheKey(targetName, methodName, arguments);  
        logger.debug("cache key is " + cacheKey);
        Element element = cache.get(cacheKey);   
  
        if (element == null) {   
            logger.debug("Hold up method , Get data from " + targetName + "."+ methodName );   
            result = invocation.proceed();   
            
            logger.debug("cache result in " +  cache.getName() + " cache ");    
            element = new Element(cacheKey, (Serializable) result);   
            cache.put(element);   
        }
        else
        {
        	logger.debug("find object in " + cache.getName() + " cache ");
        }
